...
vec3 sampleBlurred(sampler2D samp, vec2 uv){
    ...
    c += texture2D(samp, uv + px*vec2(-2.0,  0.0)).rgb * 0.06;
    c += texture2D(samp, uv + px*vec2(-1.0,  0.0)).rgb * 0.12;
    c += texture2D(samp, uv).rgb                  * 0.16;
    c += texture2D(samp, uv + px*vec2( 1.0,  0.0)).rgb * 0.12;
    c += texture2D(samp, uv + px*vec2( 2.0,  0.0)).rgb * 0.06;

    c += texture2D(samp, uv + px*vec2( 0.0, -2.0)).rgb * 0.06;
    c += texture2D(samp, uv + px*vec2( 0.0, -1.0)).rgb * 0.12;
    c += texture2D(samp, uv + px*vec2( 0.0,  1.0)).rgb * 0.12;
    c += texture2D(samp, uv + px*vec2( 0.0,  2.0)).rgb * 0.06;
...
void main(){
    ...
    float i = clamp(dot(col, vec3(0.299, 0.587, 0.114)), 0.0, 1.0);
    i = pow(1.0 - pow(i, 2.0), 1.0);
    i = (1.0 - i) * 0.85 + 0.15;

    float ghs = 0.15 * ghosting;

    vec3 r = sampleBlurred(texture0,
        vec2(x-0.014, -0.027)*0.85 +
        0.007*vec2(
            0.35*sin(1.0/7.0 + 15.0*curved.y + 0.9*t),
            0.35*sin(2.0/7.0 + 10.0*curved.y + 1.37*t)
        ) + vec2(scuv.x+0.001, scuv.y+0.001)
    ) * vec3(0.5,0.25,0.25);

    vec3 g = sampleBlurred(texture0,
        vec2(x-0.019, -0.020)*0.85 +
        0.007*vec2(
            0.35*cos(1.0/9.0 + 15.0*curved.y + 0.5*t),
            0.35*sin(2.0/9.0 + 10.0*curved.y + 1.50*t)
        ) + vec2(scuv.x, scuv.y-0.002)
    ) * vec3(0.25,0.5,0.25);

    vec3 b = sampleBlurred(texture0,
        vec2(x-0.017, -0.003)*0.85 +
        0.007*vec2(
            0.35*sin(2.0/3.0 + 15.0*curved.y + 0.7*t),
    ...
    col = clamp(col*1.3 + 0.75*col*col + 1.25*pow(col, vec3(5.0)), 0.0, 10.0);

    float vig = ((1.0 - 0.99*vignette) +
        16.0*curved.x*curved.y*(1.0-curved.x)*(1.0-curved.y));
    col *= 1.3 * pow(vig, 0.5);

    float scans = clamp(
        0.35 + 0.18 * sin(6.0 * (t*scanroll) - curved.y * resolution.y * 1.5),
        0.0, 1.0
    );
    col *= pow(scans, 0.9);
    ...
    col *= (1.0 - 0.004 *
        (sin(50.0*t + curved.y*2.0)*0.5 + 0.5));

    if (useFrame > 0.5) {
        vec4 f = texture2D(frameTex, curved);
        f.rgb = mix(f.rgb, vec3(0.5), 0.5);
        float fvig = clamp(
            512.0*curved.x*curved.y*(1.0-curved.x)*(1.0-curved.y),
            0.2, 0.8
        );
        col = mix(col, pow(abs(f.rgb), vec3(1.4))*fvig, f.a*f.a);
    }
