...
if "%cmake%"=="1" (
    echo [cmake build]
    if not "%web%"=="1" set "msvc=1" && echo [msvc compile]
) else (
...
:: ----------------------------------------------------------------------------
if "%cmake%"=="1" (
    set cmake_setup_desktop=cmake -B "%cmake_build_dir%\desktop" -DPLATFORM=Desktop
    set cmake_build_desktop=cmake --build "%cmake_build_dir%\desktop"
    set cmake_setup_web=emcmake cmake -B "%cmake_build_dir%\web" -DPLATFORM=Web
...
set cl_link=     /link /INCREMENTAL:NO /LIBPATH:"%raylib_dep%\lib\windows-msvc" raylib.lib gdi32.lib winmm.lib user32.lib shell32.lib
set cl_link_debug= /DEBUG
set cl_out=      /Fe:

set web_release=  -Os
set web_platform= -DPLATFORM_WEB
set web_link=     -lraylib -L"%raylib_dep%\lib\web" --shell-file shell.html --preload-file assets -sUSE_GLFW=3 -sTOTAL_MEMORY=67108864 -sFORCE_FILESYSTEM=1 -sASYNCIFY -sEXPORTED_FUNCTIONS=_main,requestFullscreen -sEXPORTED_RUNTIME_METHODS=HEAPF32

:: Choose Compile/Line Lines
:: ----------------------------------------------------------------------------

:: gcc-like defaults
if "%gcc%"=="1"   set compile=gcc %cc_common%
if "%clang%"=="1" set compile=clang %cc_common%
set compile_platform=%cc_platform%
set compile_link=%cc_link%
set compile_out=%cc_out% %output%.exe
set compile_debug=%cc_debug%
set compile_release=%cc_release%

:: compiler overrides
if "%msvc%"=="1" (
    set compile=%cl_common%
    set compile_platform=%cl_platform%
    set compile_link=%cl_link%
    if "%debug%"=="1" set compile_link=%cl_link% %cl_link_debug%
    set compile_debug=%cl_debug%
    set compile_release=%cl_release%
    set compile_out=%cl_out%%output%.exe
)

if "%web%"=="1" (
    set compile=emcc %cc_common%
    set compile_platform=%web_platform%
    set compile_link=%web_link%
    set compile_debug=%web_release%
    set compile_release=%web_release%
    set compile_out=%cc_out% index.html
)
